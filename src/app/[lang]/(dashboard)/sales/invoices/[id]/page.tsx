    ...
    <div className="flex flex-col gap-6 p-4 md:p-8 pt-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          <Button
            onClick={() => router.push(`/${lang}/sales/invoices/${invoice.id}/edit`)}
        ...
        {isUnpaid && (
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            >
              {isProcessingPayment ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />}
              {lang === 'uz' ? 'To\'lovni qabul qilish (Tezkor)' : 'Принять оплату (Быстро)'}
            </Button>
          </div>
        )}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="md:col-span-1 border-slate-100 shadow-sm bg-white rounded-2xl overflow-hidden">
          <CardHeader className="p-5 border-b border-slate-100 flex flex-row items-center gap-3 bg-slate-50/50">
            ...
            <div>
              <CardTitle className="text-sm font-bold text-slate-800">{lang === 'uz' ? 'Faktura ma\'lumotlari' : 'Информация о счете'}</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="p-5 space-y-4">
            <div className="space-y-1">
              ...
              <p className="font-semibold text-slate-850">{invoice.customers?.name ?? '—'}</p>
            </div>
            {invoice.sales_orders?.order_number && (
              <div className="space-y-1">
                ...
                <p className="font-semibold text-indigo-650 hover:underline cursor-pointer" onClick={() => router.push(`/${lang}/sales/orders/${invoice.order_id}`)}>
                  {invoice.sales_orders.order_number}
                </p>
              </div>
            )}
            <div className="space-y-1">
              ...
              <p className="font-semibold text-slate-850">{formatDate(invoice.issued_at)}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                {lang === 'uz' ? 'To\'lov muddati' : 'Срок оплаты'}
              </span>
              <p className="font-semibold text-slate-850">{formatDate(invoice.due_at)}</p>
            </div>
            {invoice.paid_at && (
              <div className="space-y-1">
                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                ...
                <p className="font-semibold text-emerald-705">{formatDate(invoice.paid_at)}</p>
              </div>
            )}
            <div className="space-y-1">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
              ...
              <div>
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${STATUS_STYLES[invoice.status] ?? 'bg-slate-50 text-slate-600'}`}>
            ...
            <div className="border-t border-slate-100 pt-3 space-y-2">
              <div className="flex justify-between text-xs text-slate-655">
                ...
                <span className="font-bold text-slate-850">{formatCurrency(invoice.total_amount)}</span>
              </div>
              <div className="flex justify-between text-xs text-slate-655">
                <span>{lang === 'uz' ? 'To\'langan summa:' : 'Оплаченная сумма:'}</span>
              ...
              <div className="flex justify-between text-xs text-slate-655 border-t pt-1">
                ...
                <span className={`font-black ${isUnpaid ? 'text-rose-600' : 'text-slate-850'}`}>
          ...
          <CardContent className="p-0">
            {items.length === 0 ? (
              <div className="p-12 text-center text-slate-500 text-sm">
                {lang === 'uz' ? 'Ushbu fakturaga bog\'liq mahsulotlar topilmadi' : 'Товары для этого счета не найдены'}
              </div>
            ) : (
