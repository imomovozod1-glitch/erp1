...
- Never render flag emoji (🇺🇿) — Windows has no flag glyphs and shows the letters instead. Use `<CountryFlag iso="UZ" />` (`src/components/ui/country-flag.tsx`, inline SVG from `country-flag-icons`), and add the country's import there when adding one to `PHONE_COUNTRIES`.
